...
| **`CLOUDFLARE_IMAGE_TAG`** | Pinned in `Apps/n8n-app/ops/dependencies.md` (source of truth) |
| **`CLOUDFLARE_CONFIG_PATH`** | `Apps/n8n-app/ops/cloudflared/config.yml` |
...
| **Functional**     | Source `CLOUDFLARE_IMAGE_TAG` from `Apps/n8n-app/ops/dependencies.md`.       |
...
## Data Design
- **Entities:** `Apps/n8n-app/ops/cloudflared/config.yml` for non-secret settings; tunnel token provided at runtime.
...
- **Verification:** Compare a checksum of `Apps/n8n-app/ops/cloudflared/config.yml` against the VM copy during phase 1.
