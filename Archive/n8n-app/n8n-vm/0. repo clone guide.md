...
2) Run the repo script (clones if missing, pulls if present):
```bash
~/self-hosted-server-apps/Apps/n8n-app/n8n-vm/0.\ repo\ clone.sh
```
If this is your first time, clone manually first:
```bash
...
## Update Later
Run the same script to pull updates:
```bash
~/self-hosted-server-apps/Apps/n8n-app/n8n-vm/0.\ repo\ clone.sh
```
