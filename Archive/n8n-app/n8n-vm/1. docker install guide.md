...
- **Scope:** VM-side steps only; uses repo pins in `Apps/n8n-app/requirements/vm_requirements.txt`.
...
- `Apps/n8n-app/requirements/vm_requirements.txt` has pinned Docker package versions.
...
## Troubleshooting
- **Package not found:** Confirm pinned versions in `Apps/n8n-app/requirements/vm_requirements.txt` match your OS (`ubuntu.22.04` for Ubuntu 22.04 or `debian.12` for Debian 12).
