...
Requires:
  ...
  - VM - N8N/Cloudflare/config.yml
  - VM - N8N/Cloudflare/docker-compose.yml
  - VM - N8N/Cloudflare/requirements.txt
...
if [[ -z "$CLOUDFLARE_IMAGE_TAG" || "$CLOUDFLARE_IMAGE_TAG" == "TBD" ]]; then
  echo "ERROR: CLOUDFLARE_IMAGE_TAG is not set. Update VM - N8N/Cloudflare/requirements.txt." >&2
  exit 1
fi
