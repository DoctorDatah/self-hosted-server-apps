...
```bash
sudo docker compose -f "/home/malik/self-hosted-server-apps/VM - N8N/Cloudflare/docker-compose.yml" logs -f
```
...
`VM - N8N/Cloudflare/config.yml` must match where the app is reachable.
