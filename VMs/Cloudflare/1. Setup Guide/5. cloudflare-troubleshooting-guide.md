...
```bash
sudo docker compose -f "/home/malik/self-hosted-server-apps/VMs/Cloudflare/docker-compose.yml" logs -f
```
...
`VMs/Cloudflare/config.yml` must match where the app is reachable.
