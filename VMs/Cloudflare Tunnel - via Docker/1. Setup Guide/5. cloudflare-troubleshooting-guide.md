...
```bash
sudo docker compose -f "/home/malik/self-hosted-server-apps/VMs/Cloudflare Tunnel - via Docker/docker-compose.yml" logs -f
```
...
```
Note: `install.sh` writes a `.env` file for future `docker compose` commands.
...
Verify from inside the cloudflared container:
```bash
sudo docker exec -it cloudflared-1 cat /etc/cloudflared/config.yml
sudo docker exec -it cloudflared-1 curl -I http://app:PORT
```
