...
## Recommended Name
- **Tunnel name:** `cloudflare-<name>_vm-tunnel`
...
5) Name the tunnel `cloudflare-<name>_vm-tunnel`.
6) Create the tunnel.
...
2) Add a hostname:
   ...
   - Domain: your domain (example: `example.com`)
   - Path: leave blank
3) Service URL:
   - If tunnel runs in Docker: `http://app:PORT`
   - If tunnel runs on the VM host: `http://localhost:PORT`
   - **Why:** `app:PORT` only resolves inside the Docker network. Use `localhost:PORT` only if cloudflared runs on the VM host and your app is bound to the host.
