...
- **Related docs:** Design/8. github action backup to google drive design.md

## Global Variables (select here)
| **Variable** | **Value** |
| --- | --- |
| **`RESTORE_TARGET`** | TBD (repo, VM, or database) |
| **`RESTORE_SOURCE_ID`** | TBD (backup name, tag, or timestamp) |
| **`RESTORE_TOOL`** | TBD (e.g., rclone) |
| **`GDRIVE_AUTH_METHOD`** | TBD (service account or OAuth) |
| **`GDRIVE_FOLDER_ID`** | TBD |
| **`CHECKSUM_METHOD`** | TBD (e.g., sha256) |
| **`DRY_RUN_MODE`** | TBD (on/off) |
...
- **Questions:** Finalize `RESTORE_TARGET`, `RESTORE_TOOL`, and `DRY_RUN_MODE`.
