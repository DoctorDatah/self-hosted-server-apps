...
## Front Matter
- **Title:** Package Handling Design (VM)
...
| **Type**           | **Requirement**                                               |
| ------------------ | ------------------------------------------------------------- |
| **Functional**     | Install host packages from `ops/system-packages.txt`.         |
...
- **Docker Engine/Compose:** Pin versions in `ops/system-packages.txt` or via apt preferences.

## Data Design
- **Entities:** `ops/system-packages.txt` for host inventory; `ops/package-audit.md` for audit notes.
...
- **Risks and mitigations:** Docker repo changes -> pin versions and track in `ops/package-audit.md`.
