...
## Required env
Create `VM - Manual App Deployment (Inside VM)/Coolify/.env` with the Coolify app variables, including:
- `COOLIFY_DB_USERNAME`
- `COOLIFY_DB_PASSWORD`
...
```
Note: `coolify_env_setup.sh` writes `VM - Manual App Deployment (Inside VM)/Coolify/.env` directly from Infisical.
...
## Network
This compose uses the shared `appnet` network (external). Ensure it exists first by running `VM - Manual App Deployment (Inside VM)/Installations/install_all.sh`.
