...
Use the shared `appnet` network created by `VM - Manual App Deployment (Inside VM)/Installations`.
...
## 3) Attach cloudflared to the same network (via cloudflare_install_and_setup.sh)
When you run `cloudflare_install_and_setup.sh`, it prompts:
```
...
In `VM - Manual App Deployment (Inside VM)/Cloudflare/config.yml`:
```yaml
ingress:
