...
Requires:
  ...
  - VM - Manual App Deployment (Inside VM)/install/install_all.sh has been run (Docker + deps installed)
  - VM - Manual App Deployment (Inside VM)/Cloudflare/config.yml
  - VM - Manual App Deployment (Inside VM)/Cloudflare/docker-compose.yml
  - VM - Manual App Deployment (Inside VM)/Cloudflare/requirements.txt
...
require_cmd() {
  command -v "$1" >/dev/null 2>&1 || {
    echo "ERROR: $1 is missing. Please execute the installation module first (VM - Manual App Deployment (Inside VM)/install/install_all.sh)." >&2
    exit 1
  }
}
...
docker compose version >/dev/null 2>&1 || {
  echo "ERROR: docker compose is missing. Please execute the installation module first (VM - Manual App Deployment (Inside VM)/install/install_all.sh)." >&2
  exit 1
}
...
if [[ -z "$CLOUDFLARE_IMAGE_TAG" || "$CLOUDFLARE_IMAGE_TAG" == "TBD" ]]; then
  echo "ERROR: CLOUDFLARE_IMAGE_TAG is not set. Update VM - Manual App Deployment (Inside VM)/Cloudflare/requirements.txt." >&2
  exit 1
fi
