...
```bash
sudo docker compose -f "/home/malik/self-hosted-server-apps/VM - Manual App Deployment (Inside VM)/Cloudflare/docker-compose.yml" logs -f
```
...
`VM - Manual App Deployment (Inside VM)/Cloudflare/config.yml` must match where the app is reachable.
