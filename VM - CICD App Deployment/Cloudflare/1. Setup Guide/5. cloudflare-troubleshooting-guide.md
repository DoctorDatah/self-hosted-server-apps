...
```bash
sudo docker compose -f "/home/malik/self-hosted-server-apps/VM - CICD App Deployment/Cloudflare/docker-compose.yml" logs -f
```
...
`VM - CICD App Deployment/Cloudflare/config.yml` must match where the app is reachable.
