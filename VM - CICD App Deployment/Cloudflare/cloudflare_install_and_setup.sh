...
Requires:
  ...
  - VM - CICD App Deployment/install/install_all.sh has been run (Docker + deps installed)
  - VM - CICD App Deployment/Cloudflare/config.yml
  - VM - CICD App Deployment/Cloudflare/docker-compose.yml
  - VM - CICD App Deployment/Cloudflare/requirements.txt
...
require_cmd() {
  command -v "$1" >/dev/null 2>&1 || {
    echo "ERROR: $1 is missing. Please execute the installation module first (VM - CICD App Deployment/install/install_all.sh)." >&2
    exit 1
  }
}
...
docker compose version >/dev/null 2>&1 || {
  echo "ERROR: docker compose is missing. Please execute the installation module first (VM - CICD App Deployment/install/install_all.sh)." >&2
  exit 1
}
...
if [[ -z "$CLOUDFLARE_IMAGE_TAG" || "$CLOUDFLARE_IMAGE_TAG" == "TBD" ]]; then
  echo "ERROR: CLOUDFLARE_IMAGE_TAG is not set. Update VM - CICD App Deployment/Cloudflare/requirements.txt." >&2
  exit 1
fi
