...
```bash
sudo docker compose -f "/home/malik/self-hosted-server-apps/VM Coolify/Cloudflare/docker-compose.yml" logs -f
```
...
`VM Coolify/Cloudflare/config.yml` must match where the app is reachable.
