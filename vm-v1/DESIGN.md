...
| Stage structure        | Independent folders under top-level `stages/`                                 |
...
This is why `stages/` exists as a first-class top-level folder instead of keeping flat shell scripts under `runtime/stages/`.
...
which is resolved from `inventory/aliases.yaml`.
...
Defines base settings and overlays that are merged into target config.
...
All real operational logic lives in `stages/`.
...
Performs base machine preparation.
...
After base install, the VM still needs secure access configuration before it is considered ready for remote automation.
...
### Step 4: run base install locally
...
| -------------------- | ----------------------------------------- | ---------------------------------- |
| `skills/`            | human-facing intent resolution            | better operator UX                 |
| `runtime/`           | orchestration engine                      | one place for execution logic      |
| `stages/`            | operational implementations               | all real work lives here           |
| `apps/`              | per-app definitions                       | keep runtime generic               |
| `inventory/`         | targets, groups, aliases                  | map the infrastructure             |
| `profiles/`          | reusable config layers                    | reduce duplication                 |
| `requirements/`      | readiness expectations                    | define capability checks           |
| `policies/`          | guardrails                                | prevent dangerous actions          |
...
**keep `runtime/` as the engine, keep all real operational logic in `stages/`, and let skills, operator CLI, and GitHub Actions all invoke the same system.**
