...
```bash
docker network ls
docker network inspect n8n | rg -i 'Name|Containers'
```
...
`Apps/cloudflare/config.yml` must match where the app is reachable.

Example (docker network):
```yaml
ingress:
  - hostname: coolify.arshware.com
    service: http://coolify:8000
```
