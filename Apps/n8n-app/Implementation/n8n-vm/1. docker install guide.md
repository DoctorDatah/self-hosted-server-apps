...
## Preconditions
- VM is Ubuntu 24.04 (noble) and has sudo access.
...
| **Name** | **Example** | **Notes** |
| --- | --- | --- |
| `PRIMARY_OS` | `Ubuntu 24.04` | This guide assumes Ubuntu 24.04. |
| `DOCKER_KEYRING_PATH` | `/etc/apt/keyrings/docker.gpg` | Path used by Docker official repo. |

## Install Steps (VM)
1) SSH to the VM.
2) Run the VM install script from the repo root:
```bash
./Apps/n8n-app/Implementation/n8n-vm/docker install guide.sh
```
...
## Troubleshooting
- **Package not found:** Confirm pinned versions in `Apps/n8n-app/ops/system-packages.txt` exist for Ubuntu 24.04.
