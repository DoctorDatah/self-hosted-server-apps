...
## Summary
- **Goal:** Clone this repo onto the VM so the install scripts can run.
...
2) Run the repo clone script:
```bash
./Apps/n8n-app/n8n-vm/0.\ repo\ clone.sh
```
- Simply copy paste the script in the terminal.
- Put github username
- Put PAT for passwords. (PAT is config in github settings (not repo's settings) -> n8n-vm-pat -> also added to ios password for future references)
