...
```bash
sha256sum ~/self-hosted-server-apps/Apps/n8n-app/ops/cloudflared/config.yml
```
Compare the hash to your trusted source before proceeding.
...
```
6) Run the VM install script (reads pins from `Apps/n8n-app/ops/dependencies.md`):
```bash
sudo -E "/home/malik/self-hosted-server-apps/Apps/n8n-app/n8n-vm/2. cloudflare install.sh"
```

7) Verify connection (no manual exports):
```bash
...
sudo -E docker rmi cloudflare/cloudflared:<PINNED_TAG_FROM_DEPENDENCIES>
```
