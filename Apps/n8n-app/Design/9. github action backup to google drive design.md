...
- **Related docs:** Design/8. google drive folder and service account setup design.md, Design/10. restore from google drive design.md

## Global Variables (select here)
| **Variable** | **Value** |
| --- | --- |
| **`BACKUP_TARGETS`** | `AI/`, `Design/`, `Implementation/`, `Human/`, `.github/workflows/`, `ops/` |
| **`BACKUP_SCHEDULE_CRON`** | `0 2 * * *` (daily 02:00 UTC) |
| **`BACKUP_TOOL`** | rclone |
| **`GDRIVE_AUTH_METHOD`** | Service account JSON |
| **`GDRIVE_FOLDER_ID`** | TBD |
| **`BACKUP_NAMING_SCHEME`** | `repo-{yyyy-mm-dd}-{commit}.zip` (proposed) |
| **`RETENTION_POLICY`** | Keep last 30 backups (by timestamp) |
...
| **Functional** | Write a checksum file using `CHECKSUM_METHOD`. |
| **Non-functional** | Fail the run on upload errors. |
| **Non-functional** | Enforce `RETENTION_POLICY` in Drive. |
...
- **Questions:** Confirm whether to include any VM data dumps in `BACKUP_TARGETS`.
