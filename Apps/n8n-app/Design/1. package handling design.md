...
| **Use a `requirements/` directory with environment-specific files.**       | Keeps pins explicit per environment and avoids mixed concerns. | Single `requirements.txt`. | VM pinning lives in `requirements/vm_requirements.txt`, with separate files for containers and runner. |
...
| **Type**           | **Requirement**                                               |
| ------------------ | ------------------------------------------------------------- |
| **Functional**     | Install host packages from `Apps/n8n-app/requirements/vm_requirements.txt`.         |
...
| **Functional**     | Store environment-specific requirement files under `Apps/n8n-app/requirements/`. |
| **Functional**     | Maintain separate requirement files for VM, n8n containers, and GitHub runner. |
...
- **Docker Engine/Compose:** Pin versions in `Apps/n8n-app/requirements/vm_requirements.txt` or via apt preferences.

## Data Design
- **Entities:** `Apps/n8n-app/requirements/vm_requirements.txt` for host inventory; `Apps/n8n-app/requirements/n8n_container_requirements.txt` for container pins; `Apps/n8n-app/requirements/github_runner_requirements.txt` for runner pins.
- **Layout:** `Apps/n8n-app/requirements/` holds environment-specific pinned requirement files (`vm_requirements.txt`, `n8n_container_requirements.txt`, `github_runner_requirements.txt`).
...
- **Risks and mitigations:** Docker repo changes -> pin versions and document updates in repo history.
