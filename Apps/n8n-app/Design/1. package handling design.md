...
| **Use a `requirements/` directory with environment-specific files.**       | Keeps pins explicit per environment and avoids mixed concerns. | Single `requirements.txt`. | VM pinning lives in `requirements/vm_requirments.txt`, with separate files for containers and runner. |
...
| **Type**           | **Requirement**                                               |
| ------------------ | ------------------------------------------------------------- |
| **Functional**     | Install host packages from `Apps/n8n-app/requirements/vm_requirments.txt`.         |
...
- **Docker Engine/Compose:** Pin versions in `Apps/n8n-app/requirements/vm_requirments.txt` or via apt preferences.

## Data Design
- **Entities:** `Apps/n8n-app/requirements/vm_requirments.txt` for host inventory; `Apps/n8n-app/requirements/n8n_container_requirments.txt` for container pins; `Apps/n8n-app/requirements/github_runner_requirments.txt` for runner pins; `Apps/n8n-app/ops/package-audit.md` for audit notes.
- **Layout:** `Apps/n8n-app/requirements/` holds environment-specific pinned requirement files (`vm_requirments.txt`, `n8n_container_requirments.txt`, `github_runner_requirments.txt`).
