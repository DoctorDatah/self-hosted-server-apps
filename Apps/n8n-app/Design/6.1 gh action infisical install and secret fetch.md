...
| **`CLOUDFLARE_CLIENT_ID`** | Cloudflare Access service token client ID (from Infisical) |
| **`CLOUDFLARE_CLIENT_SECRET`** | Cloudflare Access service token client secret (from Infisical) |
...
| **Functional** | Fetch Cloudflare Access service token `CLOUDFLARE_CLIENT_ID` and `CLOUDFLARE_CLIENT_SECRET` from Infisical. |
...
- **Data flow:** Runner installs CLI -> authenticates -> fetches Cloudflare Access service token credentials -> exports via `CLOUDFLARE_CREDS_OUTPUT` for deploy.
